...
ggplot(market_df, aes(x = '', y = Market_Share, fill = Group)) + 
  ...
  theme_void() +
  scale_fill_manual(values = c('TESLA' = 'lightblue',  
                               'OTHER' = '#F8766D'))
```
...
ggplot(make_model_sold, aes(x = reorder(Model, count_sold), y = count_sold )) +
  ...
  coord_flip() +
  labs(title = 'Tesla Model Sold - All Time', x = 'Models Sold', y = 'Count') +
  theme_minimal()
...
ggplot(Top_Tesla_Models_Priced, aes(x = reorder(Model, Estimated_revenue), y = Estimated_revenue )) +
  geom_col(fill = 'lightblue') +
  coord_flip() +
  labs(title = 'Estimated Revenue Per Model', y = 'Estimated Revenue', x = 'Models Sold') +
...
ggplot(tesla_sold_by_year, aes(x = Model_Year, y = Count)) +
    ...
    labs(title = 'Tesla Vehicles Sold by Year' ,x = 'Year') +
    theme_minimal()
```
...
ggplot(AVG_Range_For_Evs, aes( x = Tesla, y = AVG_Range, fill = Tesla))+
  geom_col() +
  labs(title = "AVG Electric Range (BEV)", y = 'AVG Range') +
  theme_minimal() +
  scale_fill_manual(values = c("TESLA" = "lightblue", 
                               "OTHERS" = "#F8766D")) 
```
...
ggplot(AVG_Range_For_Evs, aes( x = Tesla, y = AVG_MSRP, fill = Tesla))+
  geom_col() +
  labs(title = "AVG MSRP (BEV)", y = 'AVG MSRP') +
  theme_minimal() + 
  scale_fill_manual(values = c("TESLA" = "lightblue", 
                               "OTHERS" = "#F8766D")) 
```
...
ggplot(phev_vs_bev, aes(x = Model_Year, y = Count, color = Electric_Vehicle_Type)) +
  ...
  labs(title = 'PHEV vs BEV Trends Over Time', x = 'Model Year') +
